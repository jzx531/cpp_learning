...
template<typename T>
Node<T> *BinarySortTree<T>::insert(Node<T> *&node, T value)
{
    if (node == nullptr)
    {
...
template<typename T>
void BinarySortTree<T>::inorder(Node<T> *node)
{
    if(node == nullptr)
        return;
    inorder(node->left);
...
template<typename T>
void BinarySortTree<T>::preorder(Node<T> *node)
{
    if(node == nullptr)
        return;
    std::cout << node->value << " ";
...
template<typename T>
void BinarySortTree<T> ::postorder(Node<T> *node)
{
    if(node == nullptr)
        return;
    postorder(node->left);
...
template<typename T>
bool BinarySortTree<T>::remove(T data){
    if (root == nullptr) {
...
//参次遍历查看序列化后的线索二叉树
template<typename T>
void BinarySortTree<T>::levelOrder() {
    if (root == nullptr) {
        std::cout << "Empty tree" << std::endl;
        return;
    }
